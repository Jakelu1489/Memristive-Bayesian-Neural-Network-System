...
def get_model(net_type, inputs, outputs, priors, layer_type, activation_type, clt, clt_num, model_path):
    if net_type == "lenet":
        ...
        model = BayCNN_Pcam(outputs, inputs, priors, layer_type, activation_type, clt, clt_num)
        model = model.load_state_dict(torch.load(model_path))
        return model
    else:
...
def model_test(net, criterion, test_loader, num_ens=10, beta_type=0.1, epoch=None, num_epochs=None):
    net.eval()
    test_loss = 0.0
    accs = []
    pred_label = []
    ...
    for i, (inputs, labels) in enumerate(test_loader):
        ...
        if i >= 3000:
            print("stop here")
            # data_path = f"attack_dataset/Bay_clt_10_noise/PCAM/test_pred_label.json"
            # with open(data_path, "w") as f:
            #     json.dump(pred_label, f)
            #
            # data_path = f"attack_dataset/Bay_clt_10_noise/PCAM/test_pred_score.json"
            # with open(data_path, "w") as f:
        ...
        pred_label.append(log_outputs.max(1, keepdim=True)[1].clone().detach().data.tolist())
...
def run(dataset, net_type, seeds):
    ...
    # if use_clt:
    #     model_path = f"checkpoints/{dataset}/bayesian/model_{net_type}_{layer_type}_{activation_type}_clt_{clt_num}_1.pt"
    # else:
    ...
    if net_type == "lenet":
        ...
        model = BayCNN_Pcam(outputs, inputs, priors, layer_type, activation_type, use_clt, clt_num)
        # model.load_state_dict(torch.load(model_path, map_location=torch.device(device)))
    else:
        raise ValueError("Other Network do not supported")
    ...
    ckpt_name_clt = f"quan_checkpoints/{dataset}/bayesian/quan_model_{net_type}_{layer_type}_{activation_type}_clt_{10}_noise_2.pt"
    ...
    #         f'train_loss_noise_2.csv', t_loss, delimiter=",")
    #     np.savetxt(
    #         f'quan_checkpoints/{dataset}/bayesian/quan_model_{net_type}_{layer_type}_{activation_type}_clt_{10}'
    #         f'train_acc_noise_2.csv', t_acc, delimiter=",")
    #     np.savetxt(
    #         f'quan_checkpoints/{dataset}/bayesian/quan_model_{net_type}_{layer_type}_{activation_type}_clt_{10}'
    #         f'valid_loss_noise_2.csv', v_loss, delimiter=",")
    #     np.savetxt(
    #         f'quan_checkpoints/{dataset}/bayesian/quan_model_{net_type}_{layer_type}_{activation_type}_clt_{10}'
    #         f'valid_acc_noise_2.csv', v_acc, delimiter=",")
    # else:
    #     np.savetxt(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", default="PCAM", type=str, help="dataset=[MNIST/FashionMNIST/CIFAR10/PCAM"
                                                                       "/CIFAR100]")
