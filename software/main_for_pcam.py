...
def get_model(net_type, inputs, outputs, priors, layer_type, activation_type, clt, clt_num):
    if net_type == "lenet":
        ...
        return BayCNN_Pcam(outputs, inputs, priors, layer_type, activation_type, clt, clt_num)
    else:
...
def model_test(net, criterion, test_loader, num_ens=1, beta_type=0.1, epoch=None, num_epochs=None):
    ...
    for i, (inputs, labels) in enumerate(test_loader):
        ...
        accs.append(metric.acc(log_outputs, labels))
        preds = np.append(preds, log_outputs.cpu().numpy().argmax(axis=1))
        tar = np.append(tar, labels.cpu().numpy())

    print(preds.shape, tar.shape)

    f1, precision, recall, specificity, auc_score = metric.PCAM_evaluation_metirc(preds, tar)
    print("f1:", f1)
...
def run(dataset, net_type, seeds):
    ...
    train_loader, valid_loader, test_loader = data.get_pcam_dataloader(train_set, val_set, test_set, batch_size, num_workers)
